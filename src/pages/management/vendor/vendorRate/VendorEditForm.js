...
const VendorEditForm = ({ vendorEdit, onCancel, updateKey, setUpdateKey }) => {
  ...
  const formik = useFormik({
    initialValues: {
      zoneName: vendorEdit?.zoneNameID?.zoneName || 'None',
      zoneType: vendorEdit?.zoneTypeID?.zoneTypeName || 'None',
      vehicleType: vendorEdit?.VehicleTypeName?.vehicleTypeName || 'None',
      amount: vendorEdit?.cabAmount?.amount || 0,
      dualTripAmount: vendorEdit?.dualTripAmount?.amount || 0,
      guardPrice: vendorEdit?.guardPrice || 0
    },
    ...
    <FormikProvider value={formik}>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <form onSubmit={formik.handleSubmit}>
          <DialogTitle>
            Edit Vendor rate for <span style={{ color: 'rgb(70, 128, 255)' }}>{vendorEdit?.companyID?.company_name}</span>
          </DialogTitle>
          <Divider />
