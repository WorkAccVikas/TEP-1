...
export function downloadMonthlyReport(data, name) {
    // Remove `_id` key from each object
    const sanitizedData = data.map(({ _id, ...rest }) => rest);
  
    // Generate headers excluding 'vehicleName', 'vehicleNumber', and 'vehicleHolderName'
    const headers = [
      'Sr No',
      ...Object.keys(sanitizedData[0] || {}).filter(
        (key) => !['vehicleName', 'vehicleNumber', 'vehicleHolderName'].includes(key)
      )
    ];
    ...
    const totalRow = ['Total']; // Start with "Total"
    headers.slice(1).forEach((header, colIndex) => {
      const colValues = worksheet.getColumn(colIndex + 2).values.slice(2); // Skip header and total row
