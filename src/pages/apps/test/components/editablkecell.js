...
export default function CellEditable({ getValue: initialValue, row, column, table }) {
  ...
  console.log('columnDef', columnDef.meta);
  const onChange = (e) => {
  ...
  switch (columnDef.dataType) {
    ...
    case 'date':
      ...
      break;
    case 'zoneName':
      // const zoneInfo = columnDef.meta.zoneInfo;
      console.log('zoneInfo', columnDef.meta.zoneInfo);
      element = (
        <>
          <Formik
            initialValues={{
                ...
                <Form>
                  <FormControl fullWidth error={touched.zoneName && Boolean(errors.zoneName)}>
                    ...
                    <Select
                      ...
                      value={values.zoneName?.zoneName || ''}
                      id={`${index}-${id}`}
                      name="zoneName"
                      onChange={(e) => {
                        handleChange(e);
                        // Call onChange to update the cell value
                        // You may need to adjust the onChange to match your data structure
                        table.options.meta.updateData(row.index, id, { zoneName: e.target.value });
                      }}
                      onBlur={handleBlur}
                      label="Zone Name"
                    >
                      {columnDef.meta?.zoneOptions?.map((zone) => (
