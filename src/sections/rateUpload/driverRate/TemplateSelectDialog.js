...
const TemplateSelectDialog = ({ id, open, handleClose, setKey }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axiosServices.post('/cabRateMasterDriver/add', payload);
      ...
      >
        <CardContent>
          <Typography id="modal-modal-description">
            Upload Excel sheet with required headers to upload Driver Rate.Download Excel Template to streamline the process.
          </Typography>
        ...
        {files && filterOptions.selectedCompany && (
          <CardContent>
            <Stack direction="column" spacing={1} justifyContent="center" sx={{ px: 2.5, py: 1 }}>
              <Typography id="modal-modal-description">Select Drivers for which these Rates are applicable</Typography>
              <DriverSelection
